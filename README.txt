...
* autobump — автобампалка, следящая за положение треда на доске и
удаляющая за собой посты
...
Вайп: `./PCW.pl --mode wipe --chan Nullchan --proxy proxy/my/0chan`

Автобамп: `./PCW.pl --mode autobump --chan Nullchan`

Примечания
----------

* На нульчане в /b/ не работает постинг не ascii-символов — ошибка
`flood detect'
...
* При проблемах с цветным выводом логов нужно отредактировать файл
PCW/Core/Log.pm
...
`/usr/share/tesseract/tessdata/configs` или
...
В файле `PCW/Core/Net.pm` раскомментировать строку
`#use LWP::Protocol::socks;`.

Установить модуль `LWP::Protocol::socks` через CPAN:

`sudo cpan LWP::Protocol::socks` — если повезет, оно соберется и
заработает. Но я бы не надеялся.
