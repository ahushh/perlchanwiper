...
our %mode_config = (
    ...
    flood_limit       => 60*0.5,  #-- задержка в секундах перед повторной отправкой поста
    get_attempts      => 5,       #-- сколько раз пробовать скачать капчу, прежде чем считать прокси плохой
