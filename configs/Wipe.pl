...
our %mode_config = (
    ...
    post_limit        => 100,       #-- после успешного создания N постов прервать вайп. 0 - отключить
    send              => {
        mode          =>  0,       #-- 0 - постинг сразу после получения капчи
                                   #   1 - дожидаться ввода остальных капч и отправлять все сообщения одновременно
                                   #   2 - дожидаться ввода $max_pst_thrs штук капч и отправлять эти сообщения одновременно
    ...
    max_pst_thrs      => 50,       #-- максимальное колличество одновременно запущенных потоков с созданием постов
    max_cap_thrs      => 50,       #-- ...с получением капчи
