...
my $watch_target = {
                    ...
                    #take_last => 1,    #-- оставлять только последний (по номеру) найденный ID
                    board     => 'b',   #-- доска, на которой искать треды
                    take_last => 0,    #-- оставлять только последний (по номеру) найденный ID
...
our %mode_config = (
    ...
    proxy_attempts    => 2,       #-- сколько раз пробовать скачать капчу или отправить пост, прежде чем считать прокси плохой
);
