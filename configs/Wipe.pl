...
my $w_target_cnf = { #-- текущая конфигурация: берутся все треды со страниц 0-4
                    interval  => 30,  #-- интервал обновления списка постов
                    ...
                    threads => {
                                ...
                                pages   => [0,1],    #-- на каких страницах искать треды, в которые нужно отвечать
...
my $w_coros_cb = sub
{
    ...
    async {
        ...
        # через stats в веб-интерфейсе
        #$self->{stats}{Zqueue}{captcha} = scalar @get_coro;
        #$self->{stats}{Zqueue}{prepare} = scalar @prepare_coro;
        #$self->{stats}{Zqueue}{post} = scalar @post_coro;
        #$self->{stats}{Zqueue}{sleep} = scalar @sleep_coro;
        # в логах
...
our %mode_config = (
    ...
    send              => {
        mode          =>  0,      #-- 0 - постинг сразу после получения капчи
        ...
        wait_for_all  =>  0,      #-- 0 - скачивать и распознавать капчу во время массового постинга
                                  #-- 1 - скачивать капчу, но не распознавать
                                  #-- 2 - не скачивать и не распознавать - ждать отправки всех постов
        caps_accum    => 5,
    },
    max_pst_thrs      => 100,     #-- максимальное количество одновременно запущенных потоков с созданием постов
    ...
    get_attempts      => 3,       #-- сколько раз пробовать скачать капчу, прежде чем считать прокси плохой
    post_attempts     => 1,       #-- ... отправить пост повторно при ошибках timeout/net error/unknown error; пока что игнорирует max_post_thrs
    ...
    captcha_lifetime  => 60*40,   #-- время жизни капчи в секундах. 0 - отключить проверку
                                  #   протухшие капчи перезапрашиваются при попытки распознать капчу или создать пост
