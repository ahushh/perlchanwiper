...
our %mode_config = (
    ...
    max_pst_thrs      => 5,     #-- максимальное колличество одновременно запущенных потоков с созданием постов
    max_cap_thrs      => 5,     #-- ...с получением капчи
    ...
    post_attempts     => 2,       #-- сколько раз пробовать отправить пост, прежде чем считать прокси плохой
    prepare_attempts  => 2,       #-- сколько раз пробовать распознать капчу, прежде чем считать прокси плохой
    autoexit          => 0,       #-- завершать программу, если нет запущенных или стоящих в очереди прокси
