...
use constant THREAD   => 10027242;
use constant PASSWORD => 'fNfR3';
 
use constant BUMP_TIMEOUT => 60;
use constant INTERVAL     => 60*5; #--Проверять, нужен ли бамп, через заданный интервал

use constant SILENT_BUMP   => 1;    #-- Удалять бампы. 0 для отключения
use constant REGEXP        => '';  #-- Регэксп, по которому искать посты. e.g '.*' — все посты, 'bump' — содержащие слово бамп
use constant DELETE_TIMOUT => 60;

#-- Условие для бампа
use constant BUMP_IF => {
    #on_pages      => [0]  #-- бампать тред, когда он находится назаданных страницах
    not_on_pages  => [0], #-- бампать тред, когда он НЕ находится назаданных страницах
};
