...
my $find = { #-- текущая конфигурация: ищутся треды без текста на страницах 0-4
    proxy     => '',       #-- прокси для скачивания страниц. если не указано, используется рандомная
...
our %mode_config = (
    ...
    max_del_thrs   => 35,       #-- максимально количество запущенных потоков
    attempts       => 10,       #-- сколько раз пробывать удалить каждый пост
