...
our $msg = {
    ...
    postid  => {
        board  => 'b',
        thread => 10316407, #-- из какого треда брать номера постов
        update => 120,      #-- интервал обновления списка постов; 0 - отключить
    },
    #-- #delirium# config
    delirium => {
    ...
    string => {
        ...
        num_str  => 20,                       #-- количество счтиывемых строк за раз
        path     => "$ENV{HOME}/1.txt",   #-- путь к файлу
    },
};
...
my $img_altering = {
    ...
    mode         => 'randbytes',
    number_bytes => 20,                   #-- количество дописываемых байтов
    #-------------------------------------------------------------------------------------------------------
    #-- resize mode
